...
vep_cardiac <- cardiac_fish %>% filter(significance=='significant') %>%
  ...
  na.omit() %>% rename('qvalue' = q, 'case_frequency' = F_A, 'ctrl_frequency'=F_U, 
                       'minor_allele_OR' = OR) %>% arrange(qvalue)
```
...
cardiac_fish %>% filter(significance == 'significant') %>% 
  ...
  ggplot(aes(y=reorder(Existing_variation,-minor_allele_OR),x=minor_allele_OR)) + 
...
```{r, echo=FALSE}
kable(vep_cardiac %>% select(-SNP,-CLIN_SIG) %>%
        rename('rsid'=Existing_variation) %>% 
        select(rsid,SYMBOL,IMPACT,qvalue,
               case_frequency,ctrl_frequency,minor_allele_OR) %>%
